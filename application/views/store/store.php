  ...
  <div class="container-fluid">
    ...
    <div class="row-fluid">
                      <div class="span6 offset3">
	                        	...
	                        	<?php } ?>
		                       	<?php if($message==4){?>
	                        	<div class="alert alert-danger "><h3 style="text-align:center"> Image <strong>size </strong> must be 750 x 400 !</h3></div>
	                        	<?php } ?>
              </div>
            ...
            <table class="table table-bordered data-table">
              ...
              <?php // echo "<pre>"; print_r($location); ?>
                              <?php  foreach( $store as $row){?>
                                     <tr >
                                    <td class=" "> <img style="width:70px;" src="<?php echo base_url("uploads/".$row['st_image2']) ?>" </td>
                                    <td class=" "> <?php echo $row['st_name'] ?> </td>
                                    <td class=" "> <?php $cat=  $this->db->get_where('category',array('cat_id'=> $row['cat_id']));
          $x= $cat->row_array(); echo $x['cat_name']; ?> </td>
                                    
                                    <td class=" ">
                                    <?php if( $row['st_type']==0){echo'<button class="btn btn-info">NO</button> ';} else {echo'<button class="btn btn-success">YES</button> ';}?> </td>
                                    <td class=" "> <?php echo $row['email']?> </td>
                                    <td class=" "><?php $cat=  $this->db->get_where('location',array('loc_id'=>$row['st_location'])); $cat=$cat->row_array();echo $cat['locname']?></td>
                                     <td class=" "> <?php echo $row['mo_no']?> </td> 
                                     <td class=" "> <?php echo $row['st_date']?> </td> 
                                     <td class=" "> 
                                     <?php if( $row['st_status']==0){echo'<button class="btn btn-info">INACTIVE</button> ';} else {echo'<button class="btn btn-success">ACTIVE</button> ';}?>
                                    </td>
                                    <td>
                                        <a href="<?php echo base_url();?>index.php/admin/edit_store/<?php echo $row['st_id'] ?>" class="btn btn-primary " style="width: 42px;margin-bottom: 2px;">Edit</a>
                                         <a href="<?php echo base_url();?>index.php/admin/edit_store_image/<?php echo $row['st_id'] ?>" class="btn btn-primary " style="width: 42px;margin-bottom: 2px;">Image</a>
                                           <a href="<?php echo base_url();?>index.php/admin/delete_store/<?php echo $row['st_id'] ?>" class="btn btn-danger " onClick="return doconfirm();">Delete</a>
